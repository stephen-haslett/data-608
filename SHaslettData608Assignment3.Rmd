...
mortalityData <- read.csv(url, header = TRUE, stringsAsFactors = FALSE, sep = ',', encoding = "us-ascii")
head(mortalityData)
```
...
nationalAverageMortalityData <- mortalityData %>%
  ...
  mutate(NationalPopulation = sum(Population),
         ...
         NationalCrude.Rate = 10^5 * (NationalCount/NationalPopulation)) %>% 
  group_by(Year, ICD.Chapter, State) %>%
  mutate(SumCount = sum(Deaths),
         SumCrude.Rate = 10^5 * (SumCount/Population)) %>%
...
output$NationalCDRatePlot <- renderPlot({
    ggplot(data = nationalAverageMortalityData[nationalAverageMortalityData$State == input$Q2.1 & nationalAverageMortalityData$ICD.Chapter == input$Q2.2,], aes(x = Year, y = SumCrude.Rate)) +
      ...
      if (input$national_rate) {
        geom_line(aes(x = Year, y = NationalCrude.Rate), col = '#B22222', lwd = 1)
      }
  }, height = function() {
