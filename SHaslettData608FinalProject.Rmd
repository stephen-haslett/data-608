...
For this assignment I would like to explore Worldwide COVID 19 infection rates, deaths, and recoveries. Although this is an exhaustively studied area these days, I feel that it fits the requirements of this assignment. This assignment aims to answer the following questions:
...
glimpse(covid_data_main)
```

According to the above _glimpse()_ results, the main dataset consists of 306,429 observations, and 8 variables:

- **Sno (_Serial Number_)** The unique number of the observation. This variable is of no benefit to us so it will not be utilized.

- **ObservationDate** Date of the observation in _MM/DD/YYYY_ format.

- **Province.State** The province or state in which the observation was observed.

- **Country.Region** The country in which the observation was observed.

- **Last.Update** Date and time when the observation was last updated.

- **Confirmed** Cumulative number of confirmed COVID 19 cases up until the _Last.Update_ date.

- **Deaths** Cumulative number of COVID 19 deaths up until the _Last.Update_ date.

- **Recovered** Cumulative number of COVID 19 recoveries up until the _Last.Update_ date.

\ 

Now that we have an idea of the variables within the dataset, we will take a look at the first few observations of the data, and check for empty values.
...
The main dataset contains most of the data that is needed for the project, so we will now check for _NA_ values.
...
covid_ts_deaths %>% 
  ...
  scale_size_continuous(range = c(1, 8)) +
  labs(size = 'Recoveries', title = 'Worlwide COVID 19 Deaths') +
  theme(panel.background = element_rect(fill = '#00FFFF'),
        plot.title= element_text(hjust = 0.5),
        legend.position = 'top',
        legend.box.just = 'right',
